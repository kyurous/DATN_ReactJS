...
class BookingModal extends Component {
  ...
  async componentDidMount() {
    ...
    if (this.props.userInfo) {
      const { email, firstName, lastName, address, phonenumber, gender } = this.props.userInfo;
      this.setState({
        email: email || "",
        patientName: `${lastName || ""} ${firstName || ""}`,
        address: address || "",
        phoneNumber: phonenumber || "",
        selectedGender: this.buildGenderObject(gender), // cần thêm hàm này
      });
    }
  ...
  buildGenderObject = (genderKey) => {
    const genderList = this.buildDataGender(this.props.genders);
    return genderList.find((item) => item.value === genderKey) || null;
  };
  ...
  async componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (this.props.genders !== prevProps.genders) {
      this.setState({
        ...
        // ánh xạ lại thông tin user khi đã có genders
        if (this.props.userInfo) {
          const { email, firstName, lastName, address, phonenumber, gender } = this.props.userInfo;
          this.setState({
            email: email || "",
            ...
            address: address || "",
            phoneNumber: phonenumber || "",
            selectedGender: this.buildGenderObject(gender),
          });
        }
      });
    }
